...
ASan/UBSan mutation smoke described in `README.md`. Pull requests and pushes to
`main` run the hosted Linux/macOS/Windows CI matrix. The manually dispatched
long-fuzz workflow remains the authoritative hosted coverage-guided fuzz
checkpoint. A local macOS pass is never recorded as Windows or Linux evidence.
